...
pub struct OutboundCallsAnalyzer;

impl OutboundCallsAnalyzer {
    pub fn analyze(&self, path: &Path, content: &str, provider: &dyn LanguageProvider) -> Vec<Value> {
        ...
        if let Ok(query) = Query::new(&lang, provider.outbound_query()) {
            ...
            for m in matches {
                for capture in m.captures {
                    ...
                    results.push(json!({
                        ...
                        "description": format!("External system interaction found via: {}", name)
                    }));
                }
            }
        }
